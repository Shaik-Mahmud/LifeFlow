...
// Fetch messages for this donor
$msgStmt = $pdo->prepare("SELECT * FROM admin_donor_messages WHERE donor_id = ? ORDER BY created_at DESC");
$msgStmt->execute([$donorId]);
$messages = $msgStmt->fetchAll(PDO::FETCH_ASSOC);

// Mark as read when page is visited
$pdo->prepare("UPDATE admin_donor_messages SET status = 'Read' WHERE donor_id = ?")->execute([$donorId]);
        ...
        <div class="donor-main">
            ...
            <main class="donor-content">
                <section class="row g-4 mt-1">
                    <div class="col-xl-12" id="messages">
                        <article class="donor-panel">
                            <div class="donor-panel-heading">
                                <div>
                                    <h2>Your Messages</h2>
                                    <p>Messages from LifeFlow administrators.</p>
                                </div>
                            </div>
                                ...
                                <table class="table donor-table align-middle">
                                    ...
                                    <tbody>
                                        <?php foreach($messages as $msg): ?>
                                            <tr>
                                                <td><small><?= time_ago($msg['created_at']) ?></small></td>
                                                <td><strong><?= htmlspecialchars($msg['subject']) ?></strong></td>
                                                <td><?= htmlspecialchars($msg['message']) ?></td>
                                                <td><span class="badge bg-<?= $msg['status'] === 'Unread' ? 'danger' : 'secondary' ?>"><?= $msg['status'] ?></span></td>
                                            </tr>
                                        <?php endforeach; ?>
                                        <?php if(empty($messages)): ?>
                                            <tr><td colspan="4" class="text-center py-4 text-muted">You have no messages.</td></tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
                            </div>
